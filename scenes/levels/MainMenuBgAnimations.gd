...
@onready var pivotAccessory = $danglingAccessory/pivot
...
func pivotAnimation():
    moonTween = create_tween().set_loops()
    moonTween.tween_property(pivotAccessory, "rotation", 0.1, 5.0).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
    moonTween.tween_property(pivotAccessory, "rotation", -0.1, 5.0).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
